...
# 없으면 DB 생성
CREATE DATABASE seoul_shieldus
...
SELECT User, Host
FROM mysql.user
WHERE User = 'shieldus_user';

CREATE USER 'shieldus_user'@'localhost'
IDENTIFIED BY '비번';

GRANT ALL PRIVILEGES ON seoul_shieldus.*
TO 'shieldus_user'@'localhost';
...
# 실제 db 확인 및 계정 확인
mysql -u shieldus_user -p -h localhost -P 3306
